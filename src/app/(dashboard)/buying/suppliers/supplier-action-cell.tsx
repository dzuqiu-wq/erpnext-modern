...
export function SupplierActionCell({ supplierId, supplierName }: { supplierId: string, supplierName: string }) {
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  async function handleDelete() {
    if (!confirm(`确定要彻底删除供应商 "${supplierName}" 吗？此操作不可恢复。`)) return;
    setLoading(true);
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <Link href={`/buying/suppliers/${supplierId}/edit`}>
